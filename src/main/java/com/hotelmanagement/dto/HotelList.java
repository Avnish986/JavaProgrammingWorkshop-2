...
public class HotelList {
	...
	public void getCheapHotelRegular(String dates) throws ParseException {
	...
	public void getCheapHotelReward(String dates) throws ParseException {
		String[] range = dates.split(",");
		SimpleDateFormat dateFormat = new SimpleDateFormat("ddMMMyyyy");
		Date checkin = (Date) dateFormat.parse(range[0]);
		Date checkout = (Date) dateFormat.parse(range[1]);
		...
		for (Hotel k : hotelList) {
			...
			for (int i = 0; i < noOfDays; i++) {
				if (tempDay == 0 || tempDay == 6) {
					price += k.getRateWeekendReward();
				} else {
					price += k.getRateWeekdayReward();
				}
				tempDay = (tempDay + 1) % 7;
			}
			if (min > price) {
